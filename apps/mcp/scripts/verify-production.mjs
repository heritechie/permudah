...
try {
  ...
  const hasSlides =
    ...
    Array.isArray(res.structuredContent.slides) &&
    res.structuredContent.slides.length === 3;
  record("tools/call (valid)", Boolean(hasSlides), `title=${res.structuredContent?.title}, slides=${res.structuredContent?.slides?.length}`);
